...
class PostgreSQL_Table
{
    ...
    public function update_sequence(): bool
    {
        $quotedTable = $this->quote($this->table);
        $quotedID = $this->quote('ID');
        $query = "SELECT setval(pg_get_serial_sequence(?, 'ID'), COALESCE((SELECT MAX($quotedID) FROM $quotedTable), 1), EXISTS (SELECT 1 FROM $quotedTable));";

        return PDO_SQL::run_exec($query, [$quotedTable]) !== false;
    }
    ...
    private function create_table_query(bool $replace = true): string
    {
        ...
        " . $this->quote('ID') . " BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,";
