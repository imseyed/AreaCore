...
function jdate($format, $timestamp = '', $none = '', $time_zone = 'Asia/Tehran', $tr_num = 'fa') {
	global $localLanguage;
	if($localLanguage == "en"){   // در زبان انگلیسی تاریخ میلادی بشه
		return date($format, $timestamp);
	}
    if ($none=='en'){$tr_num='en'; $none='';}
...
function jcheckdate($jm, $jd, $jy) {
  ...
  $l_d = ($jm == 12 and ((((int)$jy + 12) % 33) % 4) == 1) ? 29 : (31 - (int) ((int)$jm / 6.5));
...
function input_to_timestamp($date,$time="0:0"){
	global $localLanguage;
	if($localLanguage == "en"){   // در زبان انگلیسی تاریخ میلادی بشه
		return strtotime("$date  $time");
	}
	
	// جایگزینی جداکننده‌ها و حذف فواصل اضافی
	$date = str_replace("-", "/", preg_replace('/\s+/', '', $date));
	$time = str_replace(",", ":", preg_replace('/\s+/', '', $time));
	// تبدیل رشته‌های تاریخ و زمان به آرایه
	$date = explode("/", $date);
	$time = explode(":", $time);
	
    if (jcheckdate(@$date[1],@$date[2],@$date[0])){
